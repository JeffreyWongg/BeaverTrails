...
export function TripSidebar({ onStopClick }: TripSidebarProps) {
  ...
  const getTravelIcon = (method: string) => {
    switch (method) {
      ...
      case 'drive':
      default: return <Car size={16} className="text-canada-red-light" />;
    }
  };
       ...
       <div className="p-6 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-start justify-between">
             ...
             <button
                ...
                title="Save as PDF"
                className="mt-1 p-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 hover:border-zinc-600 text-zinc-400 hover:text-canada-red-light transition-colors disabled:opacity-50"
             >
             ...
             <div key={day.date_offset} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-700 transition-colors">
                ...
                >
                   <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-zinc-800 flex items-center justify-center font-bold text-lg text-canada-red-light">
                         {day.date_offset}
                      </div>
                      ...
                      >
                         ...
                         <div className="space-y-3 pl-2 border-l border-zinc-700 ml-4 py-2">
                           {day.stops?.map((stop, sIdx) => (
                              ...
                              >
                                 <div className="absolute -left-3.5 w-2 h-2 rounded-full bg-zinc-700 group-hover:bg-canada-red transition-colors" />
                                 <span className="text-sm text-zinc-300 group-hover:text-white transition-colors py-1">
                                    {stop.name}
                                 </span>
                              </button>
                           ))}
                         ...
                         {day.overnight_hotel && (
                            ...
                            >
                               <BedDouble size={16} className="text-zinc-400 mt-0.5 flex-shrink-0 group-hover:text-white" />
                               <div>
                                  <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-1">Stay At</p>
                                  <p className="text-sm font-medium text-white">{day.overnight_hotel}</p>
                               </div>
                            </button>
                         )}
                      </motion.div>
